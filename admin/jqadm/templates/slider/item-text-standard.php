...
/** admin/jqadm/slider/item/text/config/suggest
 * List of suggested configuration keys in slider text panel
 *
				...
				<div v-for="(item, idx) in items" v-bind:key="idx" class="group-item card">
					...
					<div v-bind:id="'item-text-group-item-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						...
						<div class="card-tools-right">
							...
							<div v-if="item['slider.lists.siteid'] == siteid"
					...
					<div v-bind:id="'item-text-group-data-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
						...
						<div class="col-xl-6">
							...
							<?php if( ( $textTypes = $this->get( 'textTypes', map() ) )->count()  ) : ?>
								<div class="form-group row mandatory">
						...
						<div v-show="item['_ext']" class="col-xl-6 content-block secondary">
								...
								<div class="form-group row mandatory">
									...
									<div class="col-sm-8">
										<select is="select-component" required class="form-control custom-select listitem-type" tabindex="<?= $enc->attr( $this->get( 'tabindex' ) ); ?>"
											...
											v-bind:name="'<?= $enc->attr( $this->formparam( ['text', '_idx_', 'slider.lists.type'] ) ); ?>'.replace('_idx_', idx)"
											v-bind:text="'<?= $enc->html( $this->translate( 'admin', 'Please select' ) ); ?>'"
							...
							<?php else : ?>
								<input class="listitem-type" type="hidden"
									v-bind:name="'<?= $enc->attr( $this->formparam( array( 'text', '_idx_', 'slider.lists.type' ) ) ); ?>'.replace('_idx_', idx)"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input class="form-control listitem-datestart" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'text', '_idx_', 'slider.lists.datestart' ) ) ); ?>'.replace('_idx_', idx)"
							...
							<div class="form-group row optional">
								...
								<div class="col-sm-8">
									<input class="form-control listitem-dateend" type="datetime-local" tabindex="<?= $this->get( 'tabindex' ); ?>"
										v-bind:name="'<?= $enc->attr( $this->formparam( array( 'text', '_idx_', 'slider.lists.dateend' ) ) ); ?>'.replace('_idx_', idx)"
										placeholder="<?= $enc->attr( $this->translate( 'admin', 'YYYY-MM-DD hh:mm:ss (optional)' ) ); ?>"
										v-bind:readonly="item['slider.lists.siteid'] != siteid"
						...
						<div v-show="item['_ext']" class="col-xl-6 content-block secondary" v-bind:class="{readonly: item['slider.lists.siteid'] != siteid}">
							<config-table inline-template
								v-bind:readonly="item['slider.lists.siteid'] != siteid" v-bind:index="idx"
										...
										<tr v-for="(entry, pos) in items" v-bind:key="pos" class="config-item">
											<td class="config-row-key">
												<input is="auto-complete" required class="form-control" v-bind:readonly="readonly" tabindex="<?= $this->get( 'tabindex' ); ?>"
													...
													v-bind:keys="JSON.parse('<?= $enc->attr( $this->config( 'admin/jqadm/slider/item/text/config/suggest', [] ) ) ?>')"
													v-model="entry.key" />
											</td>
