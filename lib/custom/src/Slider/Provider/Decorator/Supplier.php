...
/**
 * Supplier address decorator for slider providers
 *
 * @package MShop
 * @subpackage Slider
 */
...
%13$s'
	...
	public function getConfigFE( \Aimeos\MShop\Order\Item\Base\Iface $basket ) : array
	{
		...
		try
		{
			...
			if( ( $value = $slider->getAttribute( 'supplier.code', 'delivery' ) ) != ''
	 ...
	 * @return \Aimeos\MShop\Order\Item\Base\Servir\Iface Order slider item with attributes added
	 */
		...
		array $attributes ) : \Aimeos\MShop\Order\Item\Base\Slider\Iface
	{
		if( ( $code = $attributes['supplier.code'] ) != '' )
		{
			...
			$this->setAttributes( $orderSliderItem, ['supplier.code' => $attributes['supplier.code']], 'hidden' );
			unset( $attributes['supplier.code'] );
		}

		return $this->getProvider()->setConfigFE( $orderSliderItem, $attributes );
	}
}
