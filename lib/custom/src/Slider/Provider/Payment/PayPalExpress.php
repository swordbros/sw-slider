	...
	implements \Aimeos\MShop\Slider\Provider\Payment\Iface
{
	...
	private $beConfig = array(
		...
		'paypalexpress.slider' => array(
			'code' => 'paypalexpress.slider',
			'internalcode' => 'paypalexpress.slider',
	 ...
	 * @param \Aimeos\MShop\Server\Item\Iface $sliderItem Slider item with configuration
	 * @throws \Aimeos\MShop\Slider\Exception If one of the required configuration values isn't available
	 */
	public function __construct( \Aimeos\MShop\Context\Item\Iface $context, \Aimeos\MShop\Slider\Item\Iface $sliderItem )
	{
		parent::__construct( $context, $sliderItem );
	...
	public function process( \Aimeos\MShop\Order\Item\Iface $order, array $params = [] ) : ?\Aimeos\MShop\Common\Helper\Form\Iface
	{
		...
		$type = \Aimeos\MShop\Order\Item\Base\Slider\Base::TYPE_PAYMENT;
		$sliderItem = $this->getBasketSlider( $orderBaseItem, $type, $this->getSliderItem()->getCode() );
		$this->setAttributes( $sliderItem, ['TOKEN' => $rvals['TOKEN']], 'payment/paypal' );
	...
	{
		if( ( $tid = $this->getOrderSliderItem( $order->getBaseId() )->getAttribute( 'TRANSACTIONID', 'payment/paypal' ) ) === null )
		{
			$msg = $this->getContext()->getI18n()->dt( 'mshop', 'PayPal Express: Payment transaction ID for order ID "%1$s" not available' );
			throw new \Aimeos\MShop\Slider\Exception( sprintf( $msg, $order->getId() ) );
		}
	...
	public function capture( \Aimeos\MShop\Order\Item\Iface $order ) : \Aimeos\MShop\Order\Item\Iface
	{
		...
		$type = \Aimeos\MShop\Order\Item\Base\Slider\Base::TYPE_PAYMENT;
		$sliderItem = $this->getBasketSlider( $baseItem, $type, $this->getSliderItem()->getCode() );

		if( ( $tid = $sliderItem->getAttribute( 'TRANSACTIONID', 'payment/paypal' ) ) === null )
		{
			$msg = $this->getContext()->getI18n()->dt( 'mshop', 'PayPal Express: Payment transaction ID for order ID "%1$s" not available' );
			throw new \Aimeos\MShop\Slider\Exception( sprintf( $msg, $order->getId() ) );
		}
		...
		$this->setAttributes( $sliderItem, $attributes, 'payment/paypal' );
		...
		\Psr\Http\Message\ResponseInterface $response ) : \Psr\Http\Message\ResponseInterface
	{
		...
		$this->setAttributes( $sliderItem, array( $params['txn_id'] => $params['payment_status'] ), 'payment/paypal/txn' );
		$this->setAttributes( $sliderItem, array( 'TRANSACTIONID' => $params['txn_id'] ), 'payment/paypal' );
		$this->saveOrderBase( $baseItem );
		...
		\Aimeos\MShop\Order\Item\Iface $orderItem ) : \Aimeos\MShop\Order\Item\Iface
	{
		...
		if( !isset( $params['token'] ) )
		{
			...
			throw new \Aimeos\MShop\Slider\Exception( $msg );
		}
		...
		{
			$attributes['TRANSACTIONID'] = $rvals['TRANSACTIONID'];
			$this->setAttributes( $sliderItem, array( $rvals['TRANSACTIONID'] => $rvals['PAYMENTSTATUS'] ), 'payment/paypal/txn' );
		}
	...
	public function isImplemented( int $what ) : bool
	{
		switch( $what )
		{
			case \Aimeos\MShop\Slider\Provider\Payment\Base::FEAT_CAPTURE:
			case \Aimeos\MShop\Slider\Provider\Payment\Base::FEAT_QUERY:
			case \Aimeos\MShop\Slider\Provider\Payment\Base::FEAT_CANCEL:
			case \Aimeos\MShop\Slider\Provider\Payment\Base::FEAT_REFUND:
				return true;
		}
	 ...
	 * @throws \Aimeos\MShop\Slider\Exception If request was not successful and an error was returned
	 */
	protected function checkResponse( string $orderid, string $response, string $method ) : array
	{
		...
		if( $rvals['ACK'] !== 'Success' )
		{
			...
			$this->getContext()->getLogger()->log( $msg, \Aimeos\MW\Logger\Base::WARN, 'core/slider/payment' );
			...
			if( $rvals['ACK'] !== 'SuccessWithWarning' )
			{
				...
				throw new \Aimeos\MShop\Slider\Exception( sprintf( $msg, $orderid, $short ) );
			}
		}
	 ...
	 * @return \Aimeos\MShop\Slider\Provider\Payment\Iface Same object for fluent interface
	 */
	protected function checkIPN( \Aimeos\MShop\Order\Item\Base\Iface $basket,
		array $params ) : \Aimeos\MShop\Slider\Provider\Payment\Iface
	{
		$attrManager = \Aimeos\MShop::create( $this->getContext(), 'order/base/slider/attribute' );
		...
		if( $this->getConfigValue( array( 'paypalexpress.AccountEmail' ) ) !== $params['receiver_email'] )
		{
			...
			throw new \Aimeos\MShop\Slider\Exception( sprintf( $msg, $params['receiver_email'] ) );
		}
		...
		if( $this->getAmount( $price ) != $params['payment_amount'] )
		{
			...
			throw new \Aimeos\MShop\Slider\Exception( sprintf( $msg, $params['payment_amount'], $params['invoice'] ) );
		}

		$search = $attrManager->createSearch();
		$expr = array(
			$search->compare( '==', 'order.base.slider.attribute.code', $params['txn_id'] ),
			$search->compare( '==', 'order.base.slider.attribute.value', $params['payment_status'] ),
		);
		...
		if( !$attrManager->searchItems( $search )->isEmpty() )
		{
			...
			throw new \Aimeos\MShop\Slider\Exception( sprintf( $msg, $params['txn_id'], $params['txn_status'] ) );
		}
		...
		switch( $response['PAYMENTSTATUS'] )
		{
			case 'Pending':
				if( isset( $response['PENDINGREASON'] ) )
				{
					...
					$this->getContext()->getLogger()->log( $str, \Aimeos\MW\Logger\Base::INFO, 'core/slider/payment' );
				}
	...
	protected function getOrderDetails( \Aimeos\MShop\Order\Item\Base\Iface $orderBase ) : array
	{
		...
		if( $this->getConfigValue( 'paypalexpress.slider', true ) )
		{
			foreach( $orderBase->getSlider( 'payment' ) as $slider )
			{
			...
			{
				$lastPos = 0;
				foreach( $orderBase->getSlider( 'delivery' ) as $slider )
				{
					$deliveryPrices = $this->addPrice( $deliveryPrices, $slider->getPrice() );

					$values['L_SHIPPINGOPTIONAMOUNT' . $lastPos] = number_format( $slider->getPrice()->getCosts(), 2, '.', '' );
					$values['L_SHIPPINGOPTIONLABEL' . $lastPos] = $slider->getCode();
					$values['L_SHIPPINGOPTIONNAME' . $lastPos] = $slider->getName();
					$values['L_SHIPPINGOPTIONISDEFAULT' . $lastPos] = 'true';

					$lastPos++;
				}
			}
			catch( \Exception $e ) { ; } // If no delivery slider is available
		}
	...
	/**
	 * Returns order slider item for specified base ID.
	 *
	 * @param string $baseid Base ID of the order
	 * @return \Aimeos\MShop\Order\Item\Base\Slider\Iface Order slider item
	 */
	protected function getOrderSliderItem( string $baseid ) : \Aimeos\MShop\Order\Item\Base\Slider\Iface
	{
		$type = \Aimeos\MShop\Order\Item\Base\Slider\Base::TYPE_PAYMENT;
		$basket = $this->getOrderBase( $baseid, \Aimeos\MShop\Order\Item\Base\Base::PARTS_SERVICE );

		return $this->getBasketSlider( $basket, $type, $this->getSliderItem()->getCode() );
	}
	...
	public function send( string $target, string $method, string $payload ) : string
	{
		if( ( $curl = curl_init() ) === false ) {
			throw new \Aimeos\MShop\Slider\Exception( 'Could not initialize curl' );
		}
		...
		try
		{
			...
			if( ( $response = curl_exec( $curl ) ) === false ) {
				throw new \Aimeos\MShop\Slider\Exception( sprintf( 'Sending order failed: "%1$s"', curl_error( $curl ) ) );
			}

			if( curl_errno( $curl ) ) {
				throw new \Aimeos\MShop\Slider\Exception( sprintf( 'Curl error: "%1$s" - "%2$s"', curl_errno( $curl ), curl_error( $curl ) ) );
			}
